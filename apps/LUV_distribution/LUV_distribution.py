...
def plot_rc(rc, rs, rho, save_LD, data_path, scale=True):
    ...
    if local_density.ndim == 1:
        ...
        if scale:
            ...
            ax.set_xlabel(fr'Unoccupied Volume(V/ $v$)', fontsize=24)  # , fontname='Times New Roman')
        ...
        if scale:
            ...
            ld_instring = np.array(1 - ld_instring) * (Rc*2)**2
            ld_hopped = np.array(1 - ld_hopped) * (Rc*2)**2
            ax.set_xlabel(fr'$Local\  Free\ Volume(V/ v)$', fontsize =24)#, fontname='Times New Roman')
            ax.tick_params(axis='x', labelsize=20)
            ax.tick_params(axis='y', labelsize=20)
            flag = "sumarea"
        else:
    ...
    ax.text(ave[1] + 0.3, y_position + 0.04, fr'$\Delta V / v={ave[2]-ave[0]:.02f}$', ha='center', va='bottom', fontsize=17, fontname='Times New Roman')
...
def plot_6sets(rc, rs, rho, save_LD, data_path, scale=True):
    ...
    ax.text(ave[1] + 0.3, y_position + 0.04, fr'$\Delta V / v={ave[-1]-ave[0]:.02f}$', ha='center', va='bottom', fontsize=17, fontname='Times New Roman')
...
def plot_5sets(rc, rs, rho, save_LD, data_path, scale=True, show=False):
    ...
    if local_density.ndim == 1:
        ...
        if scale:
            ...
            ld_instring = np.array(1 - ld_instring) * (Rc * 2) ** 2
            ld_hopped = np.array(1 - ld_hopped) * (Rc * 2) ** 2
            ax.set_xlabel(fr'Local Unoccupied Volume($V/ v$)', fontsize=24)  # , fontname='Times New Roman')
            ax.tick_params(axis='x', labelsize=20)
            ax.tick_params(axis='y', labelsize=20)
            flag = "sumarea"
        else:
        ...
        if scale:
            ...
            ld_instring = np.array(1 - ld_instring) * (Rc*2)**2
            ld_hopped = np.array(1 - ld_hopped) * (Rc*2)**2
            ax.set_xlabel(fr'Local Unoccupied Volume($V/ v$)   $\Delta r_{{ht}} > 2$', fontsize =24, fontname='Times New Roman')
            ax.tick_params(axis='x', labelsize=20)
            ax.tick_params(axis='y', labelsize=20)
            flag = "sumarea"
        else:
    ...
    ax.text(ave[2] + 0.3, y_position + 0.04, fr'$\Delta V / v={DeltaV:.02f}$', ha='center', va='bottom', fontsize=17, fontname='Times New Roman')
...
def plot_4sets(rc, rs, rho, save_LD, data_path, scale=True, show=False, bins=[50, 20, 10, 9]):
    ## plot one best sample to show the local density or unoccupied volume distribution
    Rc = rc
    save_fig = f"Rs_{rs:.02f}"
    rho = rho
    ...
    if local_density.ndim == 1:
        ...
        if scale:
            ...
            entire_density = np.array(1 - entire_density) * (Rc*2)**2
            ld_hopped = np.array(1 - ld_hopped) * (Rc*2)**2
            ax.set_xlabel(fr'$V/ v$') #($\Delta r_{{ht}} > {LEN} \sigma$)
            ...
            local_head_ini = local_density[:, 2]
            local_head_final = local_density[:, 3]
            ax.set_xlabel(fr'Local density ($R_c={Rc:.02f}$)')#, fontname='Times New Roman')
            ax.tick_params(axis='x')
            ax.tick_params(axis='y')
            flag = "ld"
        ...
        # ----------------------------(考虑协方差)-------------------------
        # import random
        # if len(entire_density) > len(tail):
        #     rand_A = random.sample(entire_density.tolist(),  len(tail))
        #     cov_AT = np.cov(rand_A, tail, ddof=1)
        #     std_T = np.std(tail, ddof=1)
        #     std_A = np.std(rand_A, ddof=1)
        #     std2 = np.sqrt(std_T**2+std_A**2-2*cov_AT[0,1])/np.sqrt(len(tail))
        # else:
        #     rand_T = random.sample(tail,  len(entire_density.tolist()))
        #     cov_AT = np.cov(entire_density, rand_T, ddof=1)
        ...
        n, ave, mu, std, height = plot_norm_distribution(ax,
                                                     [entire_density, ld_hopped, head,  tail],
                                                             bins,
                                                     label=[r'$V_{{A}}$(All)',  r'$V_{{J}}$(Jumped)', r'$V_{{H}}$(Heads)',  r'$V_{{T}}$(Tails)'],
                                                     color=colors, x1=X1, x2=X2, lt=['-', '--', '-', '-'], alpha_list=[1, 0.45, 1, 1], lw=4, ms=10)
        print(f'mu is {mu}')
        print(f'std is {std}')
        y_position=height
        ax.annotate(
        ...
        DeltaV = ave[3] - ave[0]
        print(f'delta TH = {ave[3]-ave[2]} v')
        ax.text(ave[0] +0.36, y_position + 0.02, fr'$\Delta \overline{{V}}_{{AT}} ={DeltaV:.02f}v $', ha='center', va='bottom')
        # ax.annotate(
        #     '',
        ...
        # )
        # DeltaV2 = ave[3] -ave[2]
        # ax.text(ave[2] + 0.6, y_position -0.09, fr'$\Delta V_{{HT}} ={DeltaV2:.02f} v$', ha='center', va='bottom',
        #         fontsize=17, fontname='Times New Roman')
        avelist.append(abs(ave[0]-ave[3]))
        prb.append(stats.norm.cdf(mu[1], loc=mu[0], scale=std[0]))
        h = ax.get_ylim()
        ax.set_ylim([0, h[1]*1.2])
        ax.legend(loc='upper right', frameon=False, handletextpad=0, borderaxespad=-0.2)
        ax.set_ylabel(r"$p(V/v)$")#, fontname='Times New Roman')
        for spine in ax.spines.values():
            spine.set_linewidth(3)
        ax.minorticks_on()
        ax.tick_params(which='both', width=2)
...
def time_reverse_symertry(rc, rs, rho, save_LD, data_path, show=False, bins=[50, 20, 10, 9], figname="F0.png"):
    ...
    if local_density.ndim > 1:
        ...
        ax.set_xlabel(fr'$\Delta V/ v$', fontsize=48,
        ...
        ax.set_ylabel(r"$p(\Delta V/v)$", fontsize=48)  # , fontname='Times New Roman')
        for spine in ax.spines.values():
            spine.set_linewidth(3)
        ax.minorticks_on()
        # ax.set_ylim([0, 1])
        ...
        std_system.append(std[1])
        save_path = f'{obj_path}'
        savefile = f'{save_LD}/Reverse_sym_{rc:.02f}.png'
        fig.savefig(savefile, dpi=300)
        print(f'{savefile}')
        if show:
            plt.show()
...
def loop_vs_4sets(rc, rs, rho, save_LD, data_path, scale=True, show=False):
    ...
    if local_density.ndim == 1:
        ...
        if scale:
            ...
            ax.set_xlabel(fr'Local Unoccupied Volume($V/ v$)', fontsize=24, fontname='Times New Roman')  # , fontname='Times New Roman')
            ax.tick_params(axis='x', labelsize=20)
            ax.tick_params(axis='y', labelsize=20)
            flag = "sumarea"
        else:
    ...
    ax.text(ave[0] +0.36, y_position + 0.01, fr'$\Delta V_{{AT}} ={DeltaV:.02f} v$', ha='center', va='bottom',
            fontsize=17, fontname='Times New Roman')
    # ax.annotate(
    #     '',
    #     xy=(ave[3], y_position-0.1), xycoords='data',
...
def plot_3sets(rc, rs, rho, save_LD, data_path, scale=True, show=False):
    ...
    if local_density.ndim == 1:
        ...
        if scale:
            ...
            ld_instring = np.array(1 - ld_instring) * (Rc*2)**2
            ld_hopped = np.array(1 - ld_hopped) * (Rc*2)**2
            ax.set_xlabel(fr'Lcal Free Volume($V/ v$)   $\Delta r_{{ht}} > 2$', fontsize =24, fontname='Times New Roman')
            ax.tick_params(axis='x', labelsize=20)
            ax.tick_params(axis='y', labelsize=20)
            flag = "sumarea"
        else:
    ...
    DeltaV = ave[2] - (1-rho)*(Rc*2)**2
    ax.text(ave[0]+0.35, y_position+0.01, fr'$\Delta V / v={DeltaV:.02f}$', ha='center', va='bottom', fontsize=17, fontname='Times New Roman')
    avelist.append(abs(ave[0]-ave[2]))
    prb.append(stats.norm.cdf(mu[1], loc=mu[0], scale=std[0]))
    h = ax.get_ylim()
    ax.set_ylim([0, h[1]*1.2])
...
def plot_shift(rc, rh, rs, rho, save_LD, Ls, scale=True):
    ...
    ax.text(ave[1] + 0.3, y_position + 0.04, fr'$\Delta V / v={ave[0]-ave[1]:.02f}$', ha='center', va='bottom', fontsize=17, fontname='Times New Roman')
    avelist.append(abs(ave[0]-ave[1]))
    prb.append(stats.norm.cdf(mu[1], loc=mu[0], scale=std[0]))
    h = ax.get_ylim()
    ax.set_ylim([0, h[1]*1.2])
...
def main(mode, rho, data_path):
    ...
    if case[mode]=='MultiTasks':
        ...
        folders = [name for name in os.listdir(
            data_path) if name.startswith(prefix)]
        for fd in folders:
            Rc = fd.split('Rc')[1].split('_')[1]
            Rc = float(Rc)
        # #plot differential local density
        #     plot_4sets(rc=Rc, rs=0.6, rho=rho,
        #             save_LD=data_path,
...
if __name__ == '__main__':
    data_path = str(get_sub_project_root() / "data/output/0805/AVE_long_string_dt_100000_Rc_6.00_rs_0.6_rh_0.8_Lst_0_Len12.00_nseg_1000")
