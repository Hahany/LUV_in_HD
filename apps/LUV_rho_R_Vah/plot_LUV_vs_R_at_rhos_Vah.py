...
def DeltaV_vs_Rc_2D(file_dict, app_path, fix_rho=False, xa=0, xb=100):
    ...
    for i, key in enumerate(file_dict.keys()):
        ...
        ax.plot(
            ...
            marker=marker, markevery=0.5, markersize=6,
...
def plot_3sets(rc, rs, rho, nseg, obj_path, scale=True, show=False, bins=[50, 20, 20], fig_name='LUV_ditr_3sets'):
    ...
    if local_density.ndim == 1:
        ...
        current_path = os.path.abspath(__file__)
        save_fig = os.path.dirname(current_path)
        savefile = f'{save_fig}/rho_{rho:.03f}_{fig_name}.png'
