...
def select_region_pbc(glass, x1, y1, regionradius, t):
    r_region = regionradius + np.max(glass.radii) + 1.0
    x_rlow = x1 - r_region
    x_rup  = x1 + r_region
    y_rlow = y1 - r_region
    ...
    # 只复制单帧，shape: (N, 4)，内存缩小 ts 倍
    frame = glass.frames[t].copy()

    if x_rlow < glass.x_low:
        x_rlowp = glass.L[0] + x_rlow
        frame[frame[:, 2] > x_rlowp, 2] -= glass.L[0]
    elif x_rup > glass.x_up:
        x_rupp = x_rup - glass.L[0]
        frame[frame[:, 2] < x_rupp, 2] += glass.L[0]

    if y_rlow < glass.y_low:
        y_rlowp = glass.L[1] + y_rlow
        frame[frame[:, 3] > y_rlowp, 3] -= glass.L[1]
    elif y_rup > glass.y_up:
        y_rupp = y_rup - glass.L[1]
        frame[frame[:, 3] < y_rupp, 3] += glass.L[1]

    return frame, x_rlow, x_rup, y_rlow, y_rup